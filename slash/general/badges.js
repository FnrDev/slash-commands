    ...
    run: async(interaction) => {
        ...
        if (badges === 'staff') {
            ...
            if (role) {
                ...
                return interaction.reply({ content: `Total Members with role **${role.name}** has \`Discord Staff\` is **${getMemberWithRole.size}**\n\n${getFilterdBadges}` })
            }
            if (staffBadges) {
        ...
        if (badges === 'partnerd') {
            ...
            if (role) {
                const getFilterdBadges = getMemberWithRole.map(r => r.user).join("\n");
                return interaction.reply({ content: `Total Members with role **${role.name}** has \`Partnerd Badge\` is **${getMemberWithRole.size}**\n\n${getFilterdBadges}` })
            }
            if (partnerdBadge) {
        ...
        if (badges === 'HYPESQUAD_EVENTS') {
            ...
            if (role) {
                const getFilterdBadges = getMemberWithRole.map(r => r.user).join("\n");
                return interaction.reply({ content: `Total Members with role **${role.name}** has \`${badges.replaceAll('_', ' ')}\` is **${getMemberWithRole.size}**\n\n${getFilterdBadges}` })
            }
            if (hypeSquadBadge) {
