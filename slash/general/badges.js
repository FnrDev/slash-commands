module.exports = {
	...
	run: async (interaction) => {
		...
		const allMembersWithBadge = (await interaction.guild.members.fetch()).filter((r) => r.user.flags?.has(badges));
		...
		if (role) {
			...
			return interaction.reply({
				content: `Total members with role **${role.name}** has \`${badges.toLowerCase()}\` is **${
					getMembersWithRole.size
		...
		interaction.reply({
			content: `There are **${
				allMembersWithBadge.size
			}** members with ${badges.toLowerCase()} badge.\n\n${filterMembers}`,
		});
	},
};
